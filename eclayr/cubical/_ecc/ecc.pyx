...
cdef class ECC:
    ...
    def __init__(self, arr_size, interval=[0., 1.], steps=32, impulse=50.):
        ...
        Args:
            ...
            impulse (float, optional): Approximated value of dirac delta. Defaults to 50.
        """
    ...
    cdef _set_cell_dims(self):
        ...
        Returns:
            cell_dims (ndarray of shape (cH*cW, )): Dimension of all cells in the cubical complex.
        """
...
cdef class ECC3d:
    ...
    def __init__(self, arr_size, interval=[0., 1.], steps=32, impulse=50.):
        """
        Args:
            arr_size (Iterable[int], optional): (D, H, W) corresponding to the depth, height and width of input array.
            interval (Iterable[float], optional): Interval of filtration values to be considered. Defaults to [0., 1.].
            steps (int, optional):  Number of discretized points. Defaults to 32.
            impulse (float, optional): Approximated value of dirac delta. Defaults to 50.
        """
        ...
        self.num_cells_D = self.cH * self.cW    # number of cells at each depth
        self.num_pix_D = self.H * self.W        # number of pixels at each depth
        self.t_min, self.t_max = interval
        self.steps = steps
        self.res = (self.t_max-self.t_min) / (steps-1)  # distance between adjacent grid points
        self.lb = self.t_min - self.res         # lower bound for not skipping gradient computation during backpropagation
    ...
    cdef _set_cell_dims(self):
        ...
        Returns:
            cell_dims (ndarray of shape (cD*cH*cW, )): Dimension of all cells in the cubical complex.
        """
        cell_dims = np.zeros([self.cD, self.cH, self.cW], dtype=dtype_float)
        cell_dims[[i for i in range(self.cD) if i%2 == 1], :, :] += 1
        cell_dims[:, [i for i in range(self.cH) if i%2 == 1], :] += 1
        cell_dims[:, :, [i for i in range(self.cW) if i%2 == 1]] += 1
        cell_dims.setflags(write=False)
        return cell_dims.flatten()
