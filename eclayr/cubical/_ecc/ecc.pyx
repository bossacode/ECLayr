...
cdef class ECC:
    ...
    cdef inline Py_ssize_t* _find_neighbor_vtx(self, Py_ssize_t cell, float dim):
        """Returns the indices of a cell's neighboring vertices. Do not use for cells that are already vertices.
        ...
        Returns:
            neighbor_vtx (Pointer[Py_ssize_t]): C array containing index of neighboring squares.
        """
    ...
    cdef inline Py_ssize_t* _find_max_vtx(self, float *vtx_filt, Py_ssize_t *neighbor_vtx, Py_ssize_t size, Py_ssize_t *num_max):
        ...
        Args:
            ...
            neighbor_vtx (Pointer[Py_ssize_t]): C array containing index of 4 neighbor vertices.
...
cdef class ECC3d:
    ...
    cdef _set_cell_dims(self):
        ...
        Returns:
            cell_dims (ndarray of shape (cub_h*cub_w, )): Dimension of all cells in the cubical complex.
        """
        cell_dims = np.zeros([self.cub_d, self.cub_h, self.cub_w], dtype=dtype_float)
        cell_dims[[i for i in range(self.cub_d) if i%2 == 1], :, :] += 1
        cell_dims[:, [i for i in range(self.cub_h) if i%2 == 1], :] += 1
        cell_dims[:, :, [i for i in range(self.cub_w) if i%2 == 1]] += 1
        cell_dims.setflags(write=False)
        return cell_dims.flatten()
